...
three rendered SVG diagrams and their Mermaid source, with navigation, zoom,
...
and the HF²L Exchange adapter. They show the normal successful paths and identify
selected recovery limitations. The [architecture limitations](../ARCHITECTURE_V3.md#current-implementation-limitations)
...
HF/JFrog transport behavior, or cyclic handoffs. Full retry, cleanup, revocation,
and failure branches are omitted to keep the main call order readable.
...
No network is used by the rendering page. Regeneration overwrites the three SVGs
