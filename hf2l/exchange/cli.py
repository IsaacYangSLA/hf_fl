...
def main():
    ...
    parser.add_argument("command", choices=("init-db", "migrate-db", "serve", "worker", "check-storage"))
    ...
    if args.command in ("init-db", "migrate-db"):
        ...
        from .migrations import initialize, migrate
        (migrate if args.command == "migrate-db" else initialize)(engine)
        engine.dispose()
        print("Exchange schema v2 initialized")
    elif args.command == "serve":
        import uvicorn
        from .api import create_app
        uvicorn.run("hf2l.exchange.api:create_app", factory=True, workers=args.workers,
        ...
        while True:
            ...
            if args.once:
                break
            time.sleep(min(settings.worker_interval * 2 ** min(failures, 16), 300))
